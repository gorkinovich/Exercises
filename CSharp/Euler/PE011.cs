...
namespace Euler {
    /// <summary>
    /// This class represents the problem 11.
    /// </summary>
    public class PE011 : IRunnable {
        /// <summary>
        ...
        public void Run () {
            ...
            var table = ParseData(DATA, SIDE);
            var result = FindProduct(table, SIZE);

            Console.WriteLine($"The greatest product of four adjacent numbers is {result}.");
        }
        ...
        int FindProduct (int[,] table, int size) {
            ...
            var rareIndexes = Tools.Sequence(size - 1, side);
            var offsets = Tools.Sequence(size);
            var candidates = new[] {
                new { Rows = fullIndexes, Columns = lessIndexes, Offsets = offsets.Select(x => (0, x)) },
                new { Rows = lessIndexes, Columns = fullIndexes, Offsets = offsets.Select(x => (x, 0)) },
                new { Rows = lessIndexes, Columns = lessIndexes, Offsets = offsets.Select(x => (x, x)) },
                new { Rows = lessIndexes, Columns = rareIndexes, Offsets = offsets.Select(x => (x, -x)) }
            };
            var query = from candidate in candidates
                        from row in candidate.Rows
                        from column in candidate.Columns
                        let product = candidate.Offsets
                                               .Select(x => table[row + x.Item1, column + x.Item2])
                                               .Product()
                        select product;
            return query.Max();
        }
        ...
        /// <summary>
        /// Parses the numbers inside an input string with the table.
        /// </summary>
